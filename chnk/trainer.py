...
class ChunkTrainer(Trainer):
    def __init__(self, 
        ...
        self.peft_type = peft_type
        self.fthl.set_parameter(self.model)
    ...
    def select_parameter(self):
        ...
        for name,p in opt_model.named_parameters():
            ...
            p.counter = self.counter
            p.chunk_num = self.chunk_num
            if p.strategy:#1
                try:
                    p.upd_ran = self.chunk_dict[p.shape[1]][self.counter-1]
                except Exception as e:
                    print(name)
                    print(len(self.chunk_dict[p.shape[1]]))
                    print(self.counter-1)
            else:
                p.upd_ran = self.chunk_dict[p.shape[0]][self.counter-1]
    ...
    def update_parameter_state(self):
        ...
        for i in range(len(optimizer_grouped_parameters)):
            self.optimizer.param_groups[i]['params'] = optimizer_grouped_parameters[i]['params']
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    if optimizer_was_run:
                        # Delay optimizer scheduling until metrics are generated
                        if not isinstance(self.lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                            if self.counter == self.chunk_num:
                                self.lr_scheduler.step()
                                self.counter = 0
                    else:
                        if self.counter == self.chunk_num:
