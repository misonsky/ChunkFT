...
class Optimizer8bit(Optimizer):
                ...
                if p in self.state:
                    ...
                    for k, v in values.items():
                        ...
                        if isinstance(v, dict):
                            is_paged = getattr(v, "is_paged", False)
                            if not is_paged:
                                self.state[key_id][k][counter]= v[counter].to(p.device)
    ...
    def hift_get_state(self, group, p, gindex, pindex):
        ...
        for state_key in self.non_castable_tensor_keys:
            if state_key in state:
                state[state_key][counter] = state[state_key][counter].to(p.device)
    @torch.no_grad()
    def hift_offload_state(self, group, p, gindex, pindex):
        ...
        for state_key in self.non_castable_tensor_keys:
            if state_key in state:
                temp_tensor = state[state_key][counter]
                self.state[key_id][state_key][counter] = temp_tensor.to("cpu")
                del temp_tensor
        with torch.no_grad():
            p.new_grad = None
