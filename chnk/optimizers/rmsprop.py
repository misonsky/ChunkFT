    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p_index, p in enumerate(group["params"]):
                key_id = self.get_state_key(p)
                counter = p.counter
                self.state[key_id]["square_avg"][counter] = square_avgs[p_index].to("cpu")
                if group["momentum"] > 0:
                    self.state[key_id]["momentum_buffer"][counter] = momentum_buffer_list[p_index].to("cpu")
                if group["centered"]:
                    self.state[key_id]["grad_avgs"][counter] = grad_avgs[p_index].to("cpu")
                with torch.no_grad():
                    p.new_grad = None
