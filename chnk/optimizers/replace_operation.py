...
def __init__(self,
    ...
    for i, param_group in enumerate(self.optimizer.param_groups):
        ...
        fp32_group ={id(p):defaultdict(dict) for p in param_group['params']}
...
def move_device(self,fp16_groups,fp32_groups):
    for group_num, param_group in enumerate(self.optimizer.param_groups):
        ...
        fp32_group = self.fp32_groups[group_num]
        counter = p.counter
        if counter not in fp32_group[id(p)]:
            for p in param_group['params']:
                counter = p.counter
                strategy = p.strategy
                upd_ran = p.upd_ran
                if strategy:
                    fp32_group[id(p)] = {counter:p[:,upd_ran[0]:upd_ran[1]].clone().float().detach().to("cpu")}
                elif p.dim()==2:
                    fp32_group[id(p)] = {counter:p[upd_ran[0]:upd_ran[1],:].clone().float().detach().to("cpu")}
                elif p.dim() ==1:
                    fp32_group[id(p)] = {counter:p[upd_ran[0]:upd_ran[1]].clone().float().detach().to("cpu")}
...
def step(self, closure=None):
    ...
    for i, group in enumerate(fp16_groups):
        ...
        for fp32_param, fp16_param in zip(fp32_groups[i], fp16_groups[i]):
            if fp16_param.new_grad is not None:
                # print(fp16_param)
                fp32_param.new_grad = fp16_param.new_grad.to(fp32_param.dtype)
    ...
    for fp32_group, fp16_group in zip(fp32_groups, fp16_groups):
        for idx, (fp32_param, fp16_param) in enumerate(zip(fp32_group, fp16_group)):
            strategy = fp16_param.strategy
            upd_ran = fp16_param.upd_ran
            ...
            if strategy:
                fp16_param[:,upd_ran[0]:upd_ran[1]].data.copy_(fp32_param.data)
            elif fp16_param.dim() == 2:
                fp16_param[upd_ran[0]:upd_ran[1],:].data.copy_(fp32_param.data)
            elif fp16_param.dim() == 1:
                fp16_param[upd_ran[0]:upd_ran[1]].data.copy_(fp32_param.data)
...
def unscale_and_clip_grads(self, total_norm, fp32_groups,apply_scale=True):
        ...
        if apply_scale:
            for group in fp32_groups:
                for param in group:
                    if param.new_grad is not None:
                        param.new_grad.data.mul_(1. / combined_scale)
