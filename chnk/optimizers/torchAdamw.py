    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group["params"]):
                key_id = self.get_state_key(p)
                counter = p.counter
                self.state[key_id]["exp_avg"][counter] = exp_avgs[i].to("cpu")
                self.state[key_id]["exp_avg_sq"][counter] = exp_avg_sqs[i].to("cpu")
                if amsgrad:
                    self.state[key_id]["max_exp_avg_sq"][counter] = max_exp_avg_sqs[i].to("cpu")
                with torch.no_grad():
                    p.new_grad = None
